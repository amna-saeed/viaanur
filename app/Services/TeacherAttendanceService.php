...
class TeacherAttendanceService
{
    ...
    {
        $studentIds = $this->scope->studentIds();
    ...
    {
        $query = $this->scope->studentsQuery()
    ...
    {
        $userIds = collect($userIds);
        $subjectIds = $this->scope->subjectIds();

        if ($userIds->isEmpty() || $subjectIds->isEmpty()) {
            return collect();
        }

        $courseIds = DB::table('subjects')
        ...
        return DB::table('lms_enrollments')
            ...
            ->whereIn('lms_enrollments.course_id', $courseIds)
