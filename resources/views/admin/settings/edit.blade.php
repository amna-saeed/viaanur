...
@section('page_heading', 'Admin Settings')
    ...
    <div class="col-lg-4">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body p-4 text-center">
                ...
                <span class="badge rounded-pill bg-primary-subtle text-primary border border-primary-subtle">Administrator</span>
                @if($admin->last_active_at)
                    <p class="text-muted small mt-3 mb-0">
                        Last active {{ $admin->last_active_at->diffForHumans() }}
                    </p>
                @endif
            </div>
        </div>
                    ...
                    <fieldset class="mb-4">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" required value="{{ old('name', $admin->name) }}" autocomplete="name">
                                @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="col-md-6">
                                <label for="email" class="form-label">Login email <span class="text-danger">*</span></label>
                                <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" required value="{{ old('email', $admin->email) }}" autocomplete="email">
